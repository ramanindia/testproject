...
{  								
	 ...
	 let actualredirectURL ='/states/index';
		 try
			{
			...
			catch (err)
			{
				...
				res.redirect('/states/index');
				 return false;
			}
		if(QueryRedirectURL !== 'undefined')
		{
					
			 Genernal.checkRecords('state_id', deleteID,'districts',req.session.user.id, function(err, results) 
			{
				 if(err)
				 {			 
						req.flash('error', err);
				 ...
				 else
				 {
					// console.log("res==",actualredirectURL);
					 ...
					 Genernal.deleteDeactiveRecord('state_id',deleteID,'states',req.session.user.id, function(err, delresults) 
						{
							 if(err)
							 {
								req.flash('error', err);
...
{  			
	 ...
	 let QueryRedirectURL = req.query.redirectURL;
	 let errorRedirectURL = '/states/index';
	 let renderHtml = 'states/edit-state.html';
	 let actualredirectURL='/states/index';
	 
    Genernal.findAll('select * from countries where status=1 and user_id="'+req.session.user.id+'"',function(err,statesrRsults)
	 {
		 if(Object.keys(statesrRsults).length ==0)
		 {
	 ...
	 Genernal.findByFieldSingleRecord('state_id', recordID,'states',req.session.user.id,function(err, results) 
		{
			if(err)
			{
				 req.flash('error', 'Something missing wrong. Invalid request. please try again');
			...
			else
			{
				...
				{	
					 if(results)
					 {
						...
						if (Object.keys(requestData).length !==0)
						{
							 req.checkBody('country_id', 'Country namee is required.').notEmpty()
							  req.checkBody('state_name', 'State Name is required.').notEmpty()
							 req.checkBody('state_name', 'State name length between 3 to 100 characters.').len(3,100);
								...
								res.render(renderHtml,
								 {
									...
									PAGETITLE:LANGTEXT.EDITSTATETITLE,csrfToken: req.csrfToken(),
									errordata : errors,
									countries:statesrRsults
								});
							}
							else
							{
								
							Genernal.checkUquieFieldWithUser('state_name',requestData.state_name,'states',req.session.user.id,recordID,'state_id',function(err, countries) 
							{
								 if(err)
								 {	
									 res.render(renderHtml,
								 ...
								 else
								 {
									 ...
									 {					 delete requestData._csrf;
														...
														let conditions = {state_id:recordID,user_id:req.session.user.id};
														Genernal.update(requestData,'states',conditions,function(err,resultdata)
														{
															if(err)
															{
																res.render(renderHtml,
...
{  								
     let requestData = req.body;
	 let redirectURL = '/states/index';
	 let renderHtml = 'states/add-state.html';
	...
	if (Object.keys(requestData).length !==0)
	{
		...
		res.render(renderHtml,
			 {
				formData :requestData,
				PAGETITLE:LANGTEXT.ADDSTATETITLE,csrfToken: req.csrfToken(),
				errordata : errors,
				countries:results,
			});
		}
		...
		{
			Genernal.checkUquieFieldWithUser('state_name', requestData.state_name	,'states',req.session.user.id,'','', function(err, countries) 
			{
				 if(err)
				 {	
					 res.render(renderHtml,
				 ...
				 else
				 {
					 ...
					 {		
								 ...
								 delete requestData._csrf;
								 requestData.state_id=UID();
								 requestData.user_id = req.session.user.id;
								 
						Genernal.save(requestData,'states',function(err,result)
							{
								if(err)
								{
									res.render(renderHtml,
	...
	else
	{
		res.render(renderHtml,
		{
			PAGETITLE:LANGTEXT.ADDSTATETITLE,countries:results,csrfToken: req.csrfToken()
		});
	}
	 });
}
...
{  
	 ...
	 */
	 ...
	 let renderHtml = 'states/states.html';
	 let limit;
	  ...
	  if (typeof req.query.search !== 'undefined') 
		{
			...
			searchkeycondition = " and CONCAT_ws('',state_name,country_name) LIKE '%"+searchkey.replace(/\s\s+/g, ' ')+"%'";
		}
	...
	*/	
	...
	if(typeof req.query.orderBy !== 'undefined' && typeof req.query.orderType !== 'undefined') 
	 ...
	 let PageSlug  = req.params.StateSlug;
	
		conditions ="where ST.user_id = '"+ req.session.user.id+"'";
		
	  var pageTitle ='All States';
	 if(PageSlug=='active')
	 {
		 pageTitle = "Active States";
		 conditions =conditions+ ' and ST.status=1';
	 }
	 else if(PageSlug=='deactive')
	 {
		 pageTitle = "Deactive States";
		 conditions =conditions+ ' and ST.status=0';
	 }
	 
	let query = "select state_id,ST.country_id as STATECOUTRYID,country_name,state_name,ST.is_delete as STATEDELETE,ST.user_id as STATEUSERID,"+
	"ST.status as STATESTATUS,ST.created AS STATECREATED from "+tableName+" as ST inner join countries as CNTY  on ST.country_id=CNTY.country_id "+conditions+searchkeycondition+defaultorderBY+limit;
	
	//console.log("query==",query);
	let totalCountQuery = "select count(*) as totalRecord from "+tableName+" as ST inner join countries as CNTY  on ST.country_id=CNTY.country_id "+conditions+searchkeycondition;	
